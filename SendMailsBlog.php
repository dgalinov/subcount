...
$queryI = "SELECT * FROM information WHERE blogSub = 1";
                                    ...
                                    while ($rowInfo = mysqli_fetch_assoc($resultInfo)) {
                                        if (mysqli_num_rows($resultNewsletter) > 0) {
                                            while ($rowNewsletter = mysqli_fetch_assoc($resultNewsletter)) {
                                                if ($rowInfo['blogCounter'] == $rowNewsletter['id']) {
                                                    ...
                                                    for ($k = 0; $k < sizeof($emailsArray); $k++) {
                                                        try {
                                                            ...
                                                            $queryCounter = "UPDATE information SET blogCounter = ".$rowInfo['blogCounter']."+1 WHERE ".$rowInfo['blogCounter']." = ".$rowNewsletter['id']." ";
                                                    ...
                                                    $queryZ = "INSERT INTO blogRecords (timeInserted, subject, content, sendFrom, sendTo) VALUES (NOW(), '" . $rowNewsletter['subject'] . "','" . $rowNewsletter['content'] . "', '" . $rowCronExplode['emails'] . "','" . $emailsInfoArray . "')";
