...
$queryInfo = "SELECT * FROM information WHERE challengeAuditSub = 1";
$queryCron = "SELECT * FROM challengeAuditCron";
$queryMail = "SELECT * FROM challengeAuditMail";
$queryEvents = "SELECT * FROM challengeAuditEvents";
echo "<h1>SendMail</h1>";
                                                    ...
                                                    if (mysqli_num_rows($resultMail) > 0) {
                                                        while ($rowMail = mysqli_fetch_assoc($resultMail)) {
                                                            ...
                                                            if ($rowCron['event'] == $rowMail['evento']) {
                                                                ...
                                                                if ($rowInfo['challengeAuditCounter'] == $rowMail['id']) {
                                                                    ...
                                                                    for ($k = 0; $k < sizeof($emailsArray); $k++) {
                                                                        try {
                                                                            ...
                                                                            $queryCounter = "UPDATE information SET challengeAuditCounter = " . $rowInfo['challengeAuditCounter'] . "+1 WHERE " . $rowInfo['challengeAuditCounter'] . " = " . $rowMail['id'] . " ";
                                                                            if ($mail->send()) {
                                                                            } else {
                                                                                echo $mail->ErrorInfo;
                                                                            }
                                                                    ...
                                                                    $queryRecords = "INSERT INTO challengeAuditRecords (timeInserted, subject, content, sendFrom, sendTo) VALUES (NOW(), '" . $rowMail['subject'] . "','" . $rowMail['content'] . "', '" . $rowCron['emails'] . "','" . $emailsInfoArray . "')";
